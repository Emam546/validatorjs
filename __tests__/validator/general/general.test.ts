import ValidatorClass, { InputRules } from "@/index";
...
describe("test validate", () => {
  test("check passes function", () => {
    ...
    expect(
      ...
      state: false,
      errors: {
        name: [{ message: StringMessages[ValidatorClass.lang], value: 123 }],
      },
    });
  });
...
test("IF THE RULE EXIST", () => {
  ...
  expect(validator.validate(1234, "string")).toStrictEqual({
    message: StringMessages[ValidatorClass.lang],
    value: 1234,
  });
});
describe("test invalid attributes", () => {
  test("test with normal object", () => {
    ...
    expect(validator.getErrors({ wrongPath: "value" })).toStrictEqual({
      wrongPath: [
        {
          message: InvalidPath[ValidatorClass.lang],
          value: "value",
        },
      ],
    });
...
describe("test self array rules merging", () => {
  test("test with normal arrays", () => {
    ...
    expect(
      ...
      "correctPath.*0": [
        {
          message: IntMessages[ValidatorClass.lang],
          value: "string",
        },
      ],
    });
  ...
  describe("use . in the object", () => {
    test("test with normal arrays", () => {
      const validator = new ValidatorClass({
        correctPath: [["string"], "array", { 0: "integer", ".": ["required"] }],
      });
      expect(
        validator.getErrors({
          correctPath: [1234],
